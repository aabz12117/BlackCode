...
export const QRScanner = ({ onScan, onClose, title = "SYSTEM SCANNING" }: QRScannerProps) => {
    ...
    const tryDecodeQR = (imageData: ImageData): string | null => {
        // Try all inversion modes for best compatibility
        const modes: Array<"dontInvert" | "onlyInvert" | "attemptBoth"> = ["dontInvert", "onlyInvert", "attemptBoth"];
        for (const mode of modes) {
            const code = jsQR(imageData.data, imageData.width, imageData.height, { inversionAttempts: mode });
    ...
    const tick = () => {
        ...
        if (videoRef.current && videoRef.current.readyState === videoRef.current.HAVE_ENOUGH_DATA) {
            ...
            if (ctx) {
                ...
                const result = tryDecodeQR(imageData);
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                // Try multiple sizes for better QR detection
                const sizes = [
